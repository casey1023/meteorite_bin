...
def intro_menu(screen, call_state):
    fpsClock = pygame.time.Clock()
    
    #screen init
    screen.fill(BLACK)
    font = pygame.font.SysFont(font__, 50)
    title = font.render("Introduction", True, WHITE)
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                return {'from': "intro_menu", 'to': 'quit'}
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if menu_button.isOver(pos):
                    return {'from': 'intro_menu', 'to': 'main_menu'}
...
if __name__=="__main__":
    ...
    screen = gameinit()
    intro_menu(screen,{'to': 'intro_menu'})
    pygame.quit()